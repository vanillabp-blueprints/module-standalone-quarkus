...
unchanged. Every test class carries `@QuarkusTest` itself; inheriting it from the base class
is not enough to make the test a bean. Everything specific to the use case belongs into the test
...
2. Add `io.vanillabp:vanillabp-quarkus-integration` and one BPMS adapter to the application.
...
4. Build no index. The classes are part of the artifact the build processes, which is the
   only reason a workflow module in a Maven module of its own needs one.
5. Add the workflow aggregate as a JPA entity with the natural ID as `@Id`, plus a Spring
   Data repository for it. If the project already has an entity for this business case, use
   it instead of adding a second one.
6. Add `WorkflowTaskHandler` with the `@WorkflowService` annotation and one `@WorkflowTask`
...
7. Add GET endpoints starting the process and showing the aggregate.
8. Copy `LoanApprovalIT` and adapt it to the use case.
